...
const createWindow = async (): Promise<void> => {
  ...
  ipcMain.removeHandler('on-buy-error');
  ...
  ipcMain.handle('on-buy-error', async (_event, data: ITicketBuyErrorData) => {
    const manager = searchManagerMap.get(data.vendor);

    if (manager === undefined || manager === null)
      throw new Error(`Can not process ticket data because vendor ${data.vendor} not found`);

    console.error(`buying tickets error`);
    console.error(data.message);

    manager.handleBuyError();
